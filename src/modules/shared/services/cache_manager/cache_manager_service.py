import json
from typing import List, Optional, Type, TypeVar, Awaitable, Callable
from abc import ABC
...
class CacheManagerService(ABC):
    ...
    def set_value(self, key: str, value: str | BaseModel | List[BaseModel], expire_in_milliseconds: Optional[int] = None) -> bool:
        if isinstance(value, list):
            if len(value) > 0 and isinstance(value[0], BaseModel):
                value = json.dumps([item.model_dump(mode="json", by_alias=True) for item in value])
            elif len(value) == 0:
                value = json.dumps([])
            ...
            if entity_class:
                try:
                    ...
                    if isinstance(data, list):
                        ...
                        mapped_data = [
                            {**item, "_id": item.get("id")} if "id" in item and "_id" not in item else item
                            for item in data
                        ]
                        return [entity_class(**item) for item in mapped_data] if mapped_data else []
                    # Mapear 'id' para '_id' para compatibilidade com EntityAdapter
