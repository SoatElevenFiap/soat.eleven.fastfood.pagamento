...
class ClientRepository(RepositoryAdapter):
    ...
    async def add_client(self, client: ClientEntity) -> ClientEntity:
        ...
        client_id = await self.__mongo_service.add_document(self.table, client.model_dump(exclude=["id"]))
        return await self.get_client(client_id)

    async def get_client(self, client_id: str) -> Optional[ClientEntity]:
        client_document = await self.__mongo_service.get_document(self.table, {"id": client_id})
        return ClientEntity(**client_document) if client_document else None

    async def get_client_by_notification_url(self, notification_url: str) -> Optional[ClientEntity]:
        client_document = await self.__mongo_service.get_document(self.table, {"notification_url": notification_url})
        return ClientEntity(**client_document) if client_document else None